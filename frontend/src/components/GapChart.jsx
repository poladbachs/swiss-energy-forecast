  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 px-3 py-2 text-xs shadow-md">
      ...
      <p className="mt-1 tabular-nums text-gray-700 dark:text-gray-300">
        import gap {fmtMW(row.point)} <span className="text-gray-400">({fmtMW(row.lower)} … {fmtMW(row.upper)})</span>
      </p>
      {row.baseGap != null && (
...
// Import gap (demand minus solar + wind + hydro + nuclear) with the conformal
// band and a zero line. Below zero: Switzerland's own generation covers
// demand. Above zero: the difference has to come from imports or storage.
// When the sliders/scenarios are off baseline, a dashed baseline ghost appears.
export default function GapChart({ forecasts, baseline, dark, onHover }) {
  ...
  const showBaseline = baseline && baseline.length === forecasts.length &&
    forecasts.some((f, i) => Math.abs(f.import_gap.point - baseline[i].import_gap.point) > 1)

  const data = forecasts.map((f, i) => ({
    t: f.timestamp,
    point: f.import_gap.point,
    lower: f.import_gap.lower,
    upper: f.import_gap.upper,
    band: f.import_gap.upper - f.import_gap.lower,
    status: f.coverage_status,
    baseGap: showBaseline ? baseline[i].import_gap.point : null,
  }))
  ...
  return (
    <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 transition-colors hover:border-gray-300 dark:hover:border-gray-600">
      <div className="flex items-baseline justify-between">
        <h2 className="font-semibold text-gray-900 dark:text-gray-100">Import dependency</h2>
        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
          <span>below zero: CH generation covers demand</span>
        </div>
      </div>
